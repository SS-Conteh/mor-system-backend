...
app.get("/api/qr/sessions", authMiddleware, async (req, res) => {
  try {
    let query = {};
    if (req.user.role === "Head Shepherd") {
      // Head Shepherd only sees sessions belonging to MOR Head Quarter —
      // their own sessions and those created by HQ group leaders.
      // Sessions from other branch shepherds or their group leaders are excluded.
      query = { branch: "MOR Head Quarter" };
...
app.get("/api/qr/sessions/:id", authMiddleware, async (req, res) => {
  try {
    ...
    // Head Shepherd may only view HQ sessions
    if (
      req.user.role === "Head Shepherd" &&
      session.branch !== "MOR Head Quarter"
    ) {
      return res.status(403).json({ error: "Access denied" });
    }
    res.json(session);
